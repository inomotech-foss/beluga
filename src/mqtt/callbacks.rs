...
fn call(interface: *const c_void, functor: impl FnOnce(&Interface) + std::panic::UnwindSafe) {
    ...
    if let Err(err) = res {
        error!("call to interface panicked: {err:?}");
    }
}
...
pub(super) fn create_completed_callback(
    ...
    move |error_code, return_code, session_present| {
        if let Ok(error) = AwsMqttError::try_from(error_code) {
            debug!("on completed triggered (rc={return_code}, session_present={session_present}): {error}");
        } else {
            debug!("on completed triggered (rc={return_code}, session_present={session_present}): {error_code}");
        }
...
pub(super) fn create_interrupted_callback(status: Arc<FairMutex<ClientStatus>>) -> impl Fn(i32) {
    move |error_code| {
        if let Ok(error) = AwsMqttError::try_from(error_code) {
            debug!("on interrupted triggered: {error}");
        } else {
            debug!("on interrupted triggered: {error_code}");
        }
...
pub(super) fn create_resumed_callback(
    ...
    move |return_code, session_present| {
        debug!("on resumed triggered (rc={return_code}, session_present={session_present})");
