...
pub use jobs::{
    Description, Job, JobDescription, JobExecutionSummaryOwned, JobInfoOwned, JobStatus, JobTag,
    JobsClient, JobsSummaryOwned, RejectedErrorCode, RejectedOwned,
};
pub use mqtt::{
...
pub enum Error {
    ...
    TunnelCreate,
    #[error("couldn't create a jobs client")]
    JobsClientCreate,
    #[error("couldn't create a job")]
    JobCreate,
    #[error("failed to receive value from the channel")]
    Receive,
    #[error("operation is rejected with the message: \"{:?}\", and code \"{:?}\"", .0.message, .0.code)]
    Rejected(RejectedOwned),
    #[error("no jobs remain for execution")]
    NoPendingJobs,
    #[error(transparent)]
    Publish(oneshot::error::RecvError),
