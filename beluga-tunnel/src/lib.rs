...
impl Tunnel {
        ...
        S: Service,
    {
        ...
        tracing::debug!(?self.client_mode, "service started");
...
pub enum ClientMode {
    /// If the service is used on the destination side - most likely the Iot
    /// device.
    Destination,
    /// If the service is used on the source side - most likely the server
    /// trying to connect to the IoT device.
    Source,
}
