...
use crate::proto::{Message as Msg, Type};
...
impl Tunnel {
    pub(crate) async fn new(packet: Publish) -> Result<Self> {
        let Notify {
            client_access_token,
            client_mode,
            region,
            services,
        } = serde_json::from_slice::<Notify>(packet.payload.as_ref())?;
    ...
    pub async fn start(Tunnel { web_socket }: Self) -> Result<()> {
        ...
        let mut service = None::<SshService>;
        let mut stream_id = 0;
        ...
        loop {
            tokio::select! {
                    ...
                    for msg in messages {
                        match msg.msg_type() {
                            ...
                            Type::StreamStart => {
                                ...
                                service = SshService::new(websocket_in, websocket_out).await?.into();
                            }
                            Type::StreamReset => {
                                return Ok(());
                            }
                ...
                res = async { if let Some(service) = service.take() {
